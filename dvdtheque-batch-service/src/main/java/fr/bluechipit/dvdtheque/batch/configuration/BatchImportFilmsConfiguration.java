...
import fr.bluechipit.dvdtheque.batch.processor.FilmProcessor;
import fr.bluechipit.dvdtheque.batch.writer.DbFilmWriter;
...
public class BatchImportFilmsConfiguration{
    ...
    public FlatFileItemReader<FilmCsvImportFormat> reader(@Value("#{jobParameters[INPUT_FILE_PATH]}") String inputFilePath) {
		...
		if (!resource.exists()) {
			throw new IllegalStateException("Input resource must exist (reader is in 'strict' mode): " + resource.getPath());
		}
